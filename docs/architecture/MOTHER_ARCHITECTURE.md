...
## Change Summary from v0.3

- introduced versioned role-based configuration storage;
- strengthened authorization and mutation security requirements;
- added page-validity states including `PAGE_INVALID` and `PAGE_NOT_PUBLISHED`;
- prohibited assumptions about WordPress's internal persisted template-assignment value;
- recorded the first Full Width implementation hypothesis as non-authoritative `H1`;
- added an explicit Production Qualification / Release Gate;
- changed PHP minimum from a premature fixed value to a runtime-qualified decision;
- clarified Persian Owner UX + translatable-string contract;
- froze V0 architecture after Owner approval.
