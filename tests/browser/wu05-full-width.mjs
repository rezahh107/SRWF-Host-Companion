...
const registrationUrl = evidence?.fixture?.registration_page_url || evidence?.fixture?.page_url;
const inboxUrl = evidence?.fixture?.inbox_page_url;
if (!registrationUrl || !inboxUrl) {
  throw new Error('WU-05 evidence must contain Registration and Inbox fixture URLs.');
}
...
async function runMatrix({ role, pageUrl, applicationSelector }) {
  const failures = [];
  const results = {};
    ...
    try {
      ...
      const raw = await page.evaluate((selector) => {
        ...
        return {
          ...
          applicationMarkerCount: document.querySelectorAll(selector).length,
        };
      }, applicationSelector);
      ...
      if (!pass) {
        failures.push(`${role} ${width}px geometry/assertion failure`);
        await page.screenshot({ path: path.join(artifactDir, `wu05-${role}-${width}-failure.png`), fullPage: true });
      }
    } catch (error) {
      failures.push(`${role} ${width}px browser failure: ${error.message}`);
      result = {
        status: 'FAIL',
      ...
      try {
        ...
        // Browser/page may already be unavailable. JSON evidence is retained.
      }
    } finally {
...
const registration = await runMatrix({
  role: 'registration',
  pageUrl: registrationUrl,
  applicationSelector: '.srwf-wu05-application-region',
});
const inbox = await runMatrix({
  role: 'inbox',
  pageUrl: inboxUrl,
  applicationSelector: '.srwf-wu05-inbox-application-region',
});
await browser.close();

evidence.viewports = registration.results;
evidence.inbox_viewports = inbox.results;
evidence.layout_hypothesis.status = registration.allPass && registration.desktopEscapePass && inbox.allPass && inbox.desktopEscapePass
  ? 'SUPPORTED_ON_PINNED_TARGET_TUPLE'
  : 'FALSIFIED_OR_NOT_PROVEN';
evidence.layout_hypothesis.falsification_basis = 'REAL_BROWSER_GEOMETRY_REGISTRATION_AND_INBOX_320_390_430_1440_RTL';

evidence.claims.canonical_assignment_render = registration.canonicalRendered ? 'PASS' : 'FAIL';
evidence.claims.full_width_geometry = registration.allPass && registration.desktopEscapePass ? 'PASS' : 'FAIL';
evidence.claims.horizontal_overflow = registration.overflowPass ? 'PASS' : 'FAIL';
evidence.claims.header_footer_navigation = registration.hostIntegrityPass ? 'PASS' : 'FAIL';
evidence.claims.rtl_frontend = registration.rtlPass ? 'PASS' : 'FAIL';
evidence.claims.inbox_canonical_assignment_render = inbox.canonicalRendered ? 'PASS' : 'FAIL';
evidence.claims.inbox_full_width_geometry = inbox.allPass && inbox.desktopEscapePass ? 'PASS' : 'FAIL';
evidence.claims.inbox_horizontal_overflow = inbox.overflowPass ? 'PASS' : 'FAIL';
evidence.claims.inbox_header_footer_navigation = inbox.hostIntegrityPass ? 'PASS' : 'FAIL';
evidence.claims.inbox_rtl_frontend = inbox.rtlPass ? 'PASS' : 'FAIL';

const bothPass = registration.allPass && registration.desktopEscapePass && inbox.allPass && inbox.desktopEscapePass;
evidence.overall_status = bothPass
  ? 'FULL_WIDTH_GEOMETRY_AUTOMATED_QUALIFIED_ON_PINNED_TARGET_TUPLE'
  : 'FAIL';
evidence.failures = [...registration.failures, ...inbox.failures];

fs.writeFileSync(evidencePath, `${JSON.stringify(evidence, null, 2)}\n`, 'utf8');

if (!bothPass) {
  console.error(JSON.stringify({ overall_status: evidence.overall_status, failures: evidence.failures }, null, 2));
  process.exit(1);
}

const geometrySummary = (results) => Object.fromEntries(viewportWidths.map((width) => [width, {
...
console.log(JSON.stringify({
  ...
  registration: geometrySummary(registration.results),
  inbox: geometrySummary(inbox.results),
  claim_limits: {
    real_gravity_flow_inbox_integration: evidence.claims.real_gravity_flow_inbox_integration,
    gpp_integration: evidence.claims.gpp_integration,
    production_host_confirmation: evidence.claims.production_host_confirmation,
  },
}, null, 2));
